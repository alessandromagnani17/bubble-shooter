...
public class HandlerAdapterMouseMoved implements EventHandler<MouseEvent> {
    ...
    public HandlerAdapterMouseMoved(final Rotate cannonRotation, final Rotate lineRotation, 
    		final Point2D shootingBubblePosition, DrawHelpLine drawHelpLine) {
    ...
    public final void handle(final MouseEvent event) {
        ...
        this.cannonRotation.setAngle(PhysicHelper.calculateAngle(this.eventPosition, this.shootingBubblePosition));
        this.lineRotation.setAngle(PhysicHelper.calculateAngle(this.eventPosition, this.shootingBubblePosition));
        
        this.checkBounds(this.eventPosition);
    }
    ...
    private void checkBounds(final Point2D eventPosition) {
    	...
    	Point2D startPointFirstLine = new Point2D(this.drawHelpLine.getHelpLine().getStartX(), this.drawHelpLine.getHelpLine().getStartY());
    	Point2D endPointSecondLine;
    	
    	angularCoefficient = PhysicHelper.calculateAngularCoefficient(startPointFirstLine, eventPosition);
    	intercepts = PhysicHelper.calculateIntercepts(startPointFirstLine, eventPosition);
		...
		if(this.drawHelpLine.getHelpBounds().intersects(this.drawHelpLine.getRightBounds()) 
			...
			startPointFirstLine = new Point2D(this.drawHelpLine.getHelpLine().getStartX(), this.drawHelpLine.getHelpLine().getStartY());
