...
public class BoundsCollisionHandler implements CollisionHandler {
    ...
    private void linkToTopWall() {
        ...
        if (!this.gridManager.getBubbleNeighbours(shootingBubble).isEmpty()) {
                CollisionHandler handler = new GridCollisionHandler(shootingBubble, this.gridManager.getBubbleNeighbours(shootingBubble)
                                                                                                    .stream().findFirst().get(), this.gridManager);
                handler.handle();
        } else {
